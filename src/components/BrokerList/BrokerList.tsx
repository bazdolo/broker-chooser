...
const BrokerList = () => {
  ...
  const [filteredBrokers, setFilteredBrokers] = useState<Broker[] | undefined>(
    exampleBrokerData
  );

  const handleInputChange = (text: string) => {
    const newFilteredBrokers = exampleBrokerData.filter((broker) =>
    ...
    <div className={styles.brokerListContainer}>
      ...
      <ul>
        {filteredBrokers?.map((broker) => (
          <BrokerListItem broker={broker} key={broker.id} />
        ))}
      </ul>
    </div>
  );
